...
if ($actionType !== '') {
    if ($actionType === 'ARCHIVE') {
        // Include both services and reservations archived/restored actions
        $where[] = "(
        ...
        // Show all SERVICE_ADD logs
        $where[] = "a.action_type = 'SERVICE_ADD'";
    } else {
        // If not 'ADD' or 'ARCHIVE', apply the specific action type filter
        $where[] = "a.action_type = ?";
        $params[] = $actionType;
        $types .= 's';
    }
...
$paramsForQuery = $params; // copy params
...
<head>
    ...
    <style>
        ...
        .action-SERVICE_ADD { background: #28a745; }       /* Green - Add */
        .action-SERVICE_UPDATE { background: #ffc107; color: #000; } /* Yellow - Edit */
        .action-SERVICE_DELETE { background: #dc3545; }       /* Red - Delete */

        /* --- Extended Styles for Other Actions --- */
        .action-USER_LOGIN { background: #17a2b8; }        /* Teal */
        .action-USER_LOGOUT { background: #6c757d; }       /* Gray */
        .action-USER_REGISTER { background: #007bff; }     /* Blue */
        .action-FAILED_LOGIN { background: #fd7e14; } /* Orange */

        .action-RESERVATION_CREATE { background: #20c997; } /* Sea Green */
        .action-CUSTOMER_CREATE { background: #0dcaf0; }    /* Sky Blue */
        .action-CHATBOT_INTERACTION { background: #6610f2; } /* Purple */
        .action-SYSTEM_INIT { background: #343a40; }        /* Dark Gray */
    ...
    background: #20c997;  /* Green shade */
    color: white;         /* Ensure text is visible */
}

.action-SERVICE_RESTORE { 
    background: #0dcaf0;  /* Blue shade */
    color: white;         /* Ensure text is visible */
}
.action-SERVICE_ARCHIVE { 
    background: #f10505ff;  /* Blue shade */
    color: white;         /* Ensure text is visible */
}
...
<div class="container py-5">
    ...
    <button type="submit" class="btn btn-primary mb-3">Export to CSV</button>
</form>
                        ...
                        <tbody>
                            <?php foreach ($auditLogs as $log): ?>
                                <tr>
                                    ...
                                    <td><span class="badge action-<?= $log['action_type'] ?>"><?= strtoupper($log['action_type']) ?></span></td>
                                    <td><?= ucfirst(htmlspecialchars($log['description'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
