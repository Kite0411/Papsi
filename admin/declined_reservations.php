...
// --- RESTORE Declined Reservation (back to pending) ---
if (isset($_GET['restore'])) {
    $id = intval($_GET['restore']);

    $stmt = $conn->prepare("SELECT * FROM reservations WHERE id = ? AND archived = 1 AND status = 'declined'");
    ...
    if ($reservation) {
        ...
        // Log audit trail
        $desc = "Declined reservation #{$reservation['id']} restored to pending by admin '{$_SESSION['username']}'.";
    ...
    header("Location: declined_reservations.php");
    exit();
}
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("
        ...
        WHERE r.id = ? AND r.archived = 1 AND r.status = 'declined'
    ");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $reservation = $stmt->get_result()->fetch_assoc();
    ...
    if ($reservation) {
        ...
        // Log audit trail
        $desc = "Declined reservation #{$reservation['id']} ({$reservation['customer_name']} - {$reservation['vehicle_make']} {$reservation['vehicle_model']}) permanently deleted by admin '{$_SESSION['username']}'.";
...
$declined_stmt = $conn->prepare("
    ...
    FROM reservations r
    JOIN customers c ON r.customer_id = c.id
    WHERE r.archived = 1 AND r.status = 'declined'
...
<title>Declined Reservations - Auto Repair Admin</title>
...
function confirmAction(id, action) {
    ...
    if (action === 'restore') {
        ...
        message.innerText = 'This will move it back to pending reservations.';
...
document.getElementById('confirmYes').onclick = function() {
    ...
    if (pendingAction.type === 'restore') {
        window.location.href = "declined_reservations.php?restore=" + pendingAction.id;
    } else if (pendingAction.type === 'delete') {
        window.location.href = "declined_reservations.php?delete=" + pendingAction.id;
    }
};
