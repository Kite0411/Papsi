...
PENDING_FILE = ROOT_DIR / 'pending_questions.csv'
ANSWERED_FILE = ROOT_DIR / 'answered_questions.json'
...
def initialize_files():
    try:
        ...
        if not PENDING_FILE.exists():
            ...
            print(f"✅ Pending file loaded: {len(pending)} questions")
...
def match_services_to_problem(diagnoses, services):
    """Enhanced matching with case-insensitive and whitespace handling"""
    if not diagnoses:
        return []
...
def save_pending_question(question):
    """Save pending question with debug logging"""
    try:
        question_clean = question.strip()
        
        print(f"\n{'='*60}")
        print(f"📝 SAVING PENDING QUESTION")
        print(f"{'='*60}")
        print(f"Question: '{question_clean}'")
        print(f"File: {PENDING_FILE}")
        print(f"Exists: {PENDING_FILE.exists()}")
        
        if PENDING_FILE.exists():
            pending = pd.read_csv(PENDING_FILE, dtype={'question': str})
            print(f"Current count: {len(pending)}")
        else:
        ...
        print(f"✅ SAVED! New count: {len(pending)}")
        print(f"{'='*60}\n")
...
def chat():
    ...
    try:
        ...
        if not reply_parts:
            print(f"🚀 No matches found, forwarding to admin...")
            ...
            if saved:
                ...
                print(f"✅ Question forwarded successfully")
...
def health():
    return jsonify({
        ...
        "version": "3.4 - Pending Questions Path Fixed"
    }), 200
...
def root():
    return jsonify({
        ...
        "version": "3.4",
        "fixes": [
            "✅ Pending questions file path fixed (ROOT_DIR)",
            "✅ AC Service matching fixed (Aircon Cleaning)",
            "✅ Payment/Business query detection",
            "✅ Improved FAQ matching with typo handling",
            "✅ Connection pooling (5 max connections)",
            "✅ Enhanced debug logging for pending questions"
        ]
    }), 200
